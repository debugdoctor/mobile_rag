...
class EmbeddingModelsScreen extends ConsumerStatefulWidget {
  const EmbeddingModelsScreen({super.key});

  @override
  ConsumerState<EmbeddingModelsScreen> createState() => _EmbeddingModelsScreenState();
}

class _EmbeddingModelsScreenState extends ConsumerState<EmbeddingModelsScreen> {
  ...
  Future<void> _load() async {
    ...
    final config = await storage.getEmbeddingConfig();
    if (!mounted) {
      return;
    }
    setState(() {
  ...
  void _scheduleSave() {
    ...
    _debounce = Timer(const Duration(milliseconds: 400), () async {
      ...
      try {
        await storage.setEmbeddingConfig(
  ...
  Future<void> _updateModels(List<String> next, String? selected) async {
    final storage = ref.read(storageServiceProvider);
    await storage.setEmbeddingConfig(models: next, model: selected ?? _activeModel);
    await _load();
  }
  ...
  Future<void> _testModel(String id) async {
    ...
    try {
      ...
      await api.requestEmbeddings(
        EmbeddingRequestOptions(input: 'hello', model: id),
      );
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(translate(locale, 'settings.embedding.title'))),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          Text(translate(locale, 'settings.models.embeddingDesc')),
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(translate(locale, 'settings.embedding.connection')),
